...
int main()
{
    int i , j , k , size , index , indexage ;
    int flag=0;
    int n=0;
    char ch;
    char menu [9][300] = {"NEW ","DISPLAY ALL ID","DISPLAY ONE ID ONLY","DISPLAY ALL age","DISPLAY One age","EXIT"};
    printf("Enter your number of Employees  :");
    scanf("%d",&size);
    struct emp* ptr=(struct emp*)malloc(size*sizeof(struct emp));
    int* ptrid=(int*)malloc(size*sizeof(int));
    int* ptrage=(int*)malloc(size*sizeof(int));
    for(j=0;j<size;j++)
    {
        ptrid[j]= -1;
        ptrage[j]= -1;
    ...
    do
    {
        ...
        case 13 :
            ...
            {
                system("cls");
        printf("ENTER YOUR INDEX ID:");
        scanf("%d",&index);
        printf("Enter your id : ");
        scanf("%d",&ptr[index].id);
        ptrid[index]=ptr[index].id;
        printf("ENTER YOUR INDEX Age:");
        scanf("%d",&indexage);
        printf("Enter your age : ");
        scanf("%d",&ptr[indexage].age);
        ptrage[indexage]=ptr[indexage].age;
        /*for(j=0;j<size;j++)
        {
